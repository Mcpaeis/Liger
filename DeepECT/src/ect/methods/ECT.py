...
class ECTnode(torch.nn.Module):
    """
    The implementation for the ECTtree, warning this pytorch module cannot be saved yet
    """

    def __init__(self, node_id: int, tree: 'ECTree', center_value_np,
                 split_level,
                 left_child,
                 right_child):
        super().__init__()
    ...
    @classmethod
    def new_leaf_node(cls, node_id: int, tree: 'ECTree', center_value_np):
        return cls(node_id, tree, center_value_np, None, None, None)

    @classmethod
    def new_split_node(cls, node_id: int, tree: 'ECTree', split_level: int,
                       left_child: 'ECTnode',
                       right_child: 'ECTnode'):
    ...
    def split_node(self, split_level: int, left_child: 'ECTree', right_child: 'ECTree'):
        self.split_level = split_level
        self.left_child = left_child
        self.right_child = right_child
...
class ECTree(torch.nn.Module):
    ...
    def _create_root_node(self, root_centers=None):
        ...
        self.root_node: ECTnode = ECTnode.new_split_node(0, self, 1,
                                                         ECTnode.new_leaf_node(1, self,
                                                                               root_centers[0,
                                                                               :]),
                                                         ECTnode.new_leaf_node(2, self,
                                                                               root_centers[1,
                                                                               :]))
