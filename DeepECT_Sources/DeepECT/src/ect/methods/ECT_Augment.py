...
class ECTnodeAugment(torch.nn.Module):
    """
    The implementation for the ECTtree with augemntation, warning this pytorch module cannot be saved yet
    """

    def __init__(self, node_id: int, tree: 'ECTreeAugment', center_value_np,
                 split_level,
                 left_child,
                 right_child):
        super().__init__()
    ...
    @classmethod
    def new_leaf_node(cls, node_id: int, tree: 'ECTnodeAugment', center_value_np):
        return cls(node_id, tree, center_value_np, None, None, None)

    @classmethod
    def new_split_node(cls, node_id: int, tree: 'ECTnodeAugment', split_level: int,
                       left_child: 'ECTnodeAugment',
                       right_child: 'ECTnodeAugment'):
    ...
    def split_node(self, split_level: int, left_child: 'ECTnodeAugment', right_child: 'ECTnodeAugment'):
        self.split_level = split_level
        self.left_child = left_child
        self.right_child = right_child
    ...
    def _update_center_buffer(self):
        ...
        # updating the projection direction
        diff = child_centers[1, :] - child_centers[0, :]
        self.projection_dir = (diff / diff.norm()).detach().data.unsqueeze(1)
    ...
    def _projected_centers_and_data(self, data_batch):
        ...
        direction_vec = self.projection_dir
        projected_centers = self.child_centers().matmul(direction_vec)
        projected_data = data_batch.matmul(direction_vec)
    ...
    def loss_centers_fixed_data(self, data_batch, batch_data_aug, assigned_to_left, assigned_to_right):
        self._check_not_leaf()

        data_batch_fixed = data_batch.detach()
        batch_data_aug_fixed = batch_data_aug.detach()

        n_dps_left = assigned_to_left.sum(0).detach()
        if self.left_child.is_leaf() and n_dps_left > 0.0:
            batch_mean_left = (assigned_to_left.unsqueeze(1) * data_batch_fixed).sum(0) \
                              + (assigned_to_left.unsqueeze(1) * batch_data_aug_fixed).sum(0)
            batch_mean_left = batch_mean_left / (2.0 * n_dps_left)
        ...
        if self.right_child.is_leaf() and n_dps_right > 0.0:
            batch_mean_right = (assigned_to_right.unsqueeze(1) * data_batch_fixed).sum(0) \
                               + (assigned_to_right.unsqueeze(1) * batch_data_aug_fixed).sum(0)
            batch_mean_right = batch_mean_right / (2.0 * n_dps_right)
...
class ECTreeAugment(torch.nn.Module):
    ...
    def _create_root_node(self, root_centers=None):
        ...
        self.root_node: ECTnodeAugment = ECTnodeAugment.new_split_node(0, self, 1,
                                                                                   ECTnodeAugment.new_leaf_node(1,
                                                                                                                      self,
                                                                                                                      root_centers[
                                                                                                                      0,
                                                                                                                      :]),
                                                                                   ECTnodeAugment.new_leaf_node(2,
                                                                                                                      self,
                                                                                                                      root_centers[
                                                                                                                      1,
                                                                                                                      :]))
    ...
    def prediction(self, n_clusters, batch_data):
        ...
        # This is quite nice for comparing visualisations of different hierarchy levels
        if 2 < n_clusters > self.n_splits + 1:
            raise RuntimeError(f"level {n_clusters} is not within 2 and {self.n_splits + 1}")
    ...
    def split_highest_sse_node(self):
        ...
        new_left_leaf = ECTnodeAugment.new_leaf_node(self.next_free_node_id, self, init_centers[0, :])
        new_right_leaf = ECTnodeAugment.new_leaf_node(self.next_free_node_id + 1, self, init_centers[1, :])
    ...
    def loss(self, batch_data, batch_data_aug, is_training):
        labels, _, node_id_label_map = self.leaf_prediction(batch_data)
        ...
        if pruning_necessary:
            self._prune_tree()
            return torch.tensor([0.0], device=self.device), torch.tensor([0.0], device=self.device), torch.tensor([0.0],
                                                                                                                  device=self.device)
        else:
            center_losses = 0.0
            dp_losses = 0.0
            ...
            def recursive(node):
                ...
                augmentation_loss += node.loss_data_fixed_centers(batch_data_aug, assigned_to_left, assigned_to_right)
                center_node_loss = node.loss_centers_fixed_data(batch_data, batch_data_aug, assigned_to_left,
                                                                assigned_to_right)
                dp_losses += node.loss_data_fixed_centers(batch_data, assigned_to_left, assigned_to_right)
            ...
            return dp_loss, center_losses, augmentation_loss
