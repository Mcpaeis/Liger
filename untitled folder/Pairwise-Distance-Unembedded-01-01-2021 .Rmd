---
title: "Pairwise-Distance-Unembedded-01-01-2021"
...
```{r}
aa = (as.data.frame(t(pbmc_10X_scale_var)))
head(aa)
```

```{r}
bb = (as.data.frame(t(pbmc_SqW_scale_var)))
head(bb)
```
...
computeTripletLoss <- function(embedding, clusters){
  ...
  embedding <- as.data.frame(t(embedding))
...
library(tidyverse)
# merge the dataset
pbmc_10XSqWMergedTPL <- cbind(pbmc_10X_scale_var, pbmc_SqW_scale_var)
#write.csv(pbmc_10XSqWMergedTPL, "/Users/sixtusdakurah/Desktop/liger/data/unembedded_merged_embedding.csv")
tpl_df <- computeTripletLoss(pbmc_10XSqWMergedTPL, original_clusters)
```
...
```{r}
write.csv(tpl_df, "/Users/sixtusdakurah/Desktop/liger/data/tpl_unembedded_merged_embedding.csv")
```
